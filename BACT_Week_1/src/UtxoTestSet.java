...
/**
 * UtxoTestSet represent a random created data test set for testing isValid and
 * txHandler methods. Creates an UtxPool ans several transactions, valid,
 * invalid or conflicted Several flags force different types of transactions
 * 
 * A UtxoTestSet can be constructed wit by means of a builder:
 * 
 * <pre>
 * UtxoTestSet.builder().setPeopleSize(10).setUtxoTxNumber(10).setMaxUtxoTxOutput(10).setMaxValue(200).setTxPerTest(10)
 * 		.setMaxInput(10).setMaxOutput(10).setCorruptedPercentage(0).build();
 * </pre>
 * 
 * @author ealbertos
 *
...
public class UtxoTestSet {
	...
	/**
	 * Validation list contains list with valid transactions, invalid
	 * transactions and conflicted transactions
	 * 
	...
	static class UtxoTestSetBuilder {
		 ...
		 * 
		 * @param peopleNumber
		 *            the number of different people address in the test set
		 * @return this builder
		 */
		...
		/**
		 * Number of utxo in the pool to create for the set, the same number is
		 * created for the extraPool set Extra Pool set is used to create
		 * transactions tha use UTXO no in the actual pool
		 * 
		 * @param utxoTxNumber
		 *            number of utxo in the pool to create for the set
		 * @return this builder
		 */
		 ...
		 * 
		 * @param maxInputs
		 *            max number of inputs in transactions created
		 * @return
		 */
		 ...
		 * 
		 * @param maxOutputs
		 *            max number of outputs in transactions created
		 * @return this builder
		 */
		...
		/**
		 * Create transactions containing signatures using incorrect private
		 * keys
		 * 
		 * @param value
		 * @return this builder
		...
		/**
		 * Create transactions whose total output value exceeds total input
		 * value
		 * 
		 * @param value
		 *            True force creation
		...
		public UtxoTestSet build() throws NoSuchAlgorithmException, InvalidKeyException, SignatureException {
			return new UtxoTestSet(peopleSize, utxoTxNumber, maxUtxoTxOutput, maxValue, txNumberPerTest, maxInputs,
					maxOutputs, corruptedPercentage, isForceInvalidPrivateKeys, isForceInvalidTotals,
					isClaimingOutputsNotInPool, isForceCorruptedSignature, isClaimingUtxoSeveralTimes,
	...
	private Transaction createTxWithOutputs(List<KeyPair> people, double maxValue, Map<Integer, KeyPair> keyPairAtIndex,
			int num) {
	...
	private void addTxOutputsToPool(UTXOPool utxoPool, Map<Integer, KeyPair> keyPairAtIndex,
			Map<UTXO, KeyPair> utxoToKeyPair, int numTx, Transaction tx) {
	...
	private byte[] sign(PrivateKey privateKey, byte[] rawDataToSign)
			throws NoSuchAlgorithmException, InvalidKeyException, SignatureException {
	...
	private ValidationLists<Transaction> generateTrxWithCorruptedSignaturePercentage()
		...
		for (int i = 0; i < txNumberPerTest; i++)
		{
			...
			{
				double value;
				if ((isForceInvalidTotals && isRandomSelection()) || outputValue > inputValue)
				{
		...
		return ValidationLists.builder(Transaction.class).setValid(valid).setInvalid(invalid).setConflicted(conflicted)
				.build();
