...
public class Transaction {
	...
	/**
	 * this method prepares the byte array to be signed from the input and the
	...
	public byte[] getRawDataToSign(int index) {
		...
		if (index > inputs.size())
			return null;
		Input in = inputs.get(index); //get input object at that index
		byte[] prevTxHash = in.prevTxHash; //get the transaction from where it is coming 
		ByteBuffer b = ByteBuffer.allocate(Integer.SIZE / 8); //get a buffer
		b.putInt(in.outputIndex); //put index of the transaction in the buffer 
		byte[] outputIndex = b.array(); //prepare array
		if (prevTxHash != null)
			for (int i = 0; i < prevTxHash.length; i++)
				sigData.add(prevTxHash[i]); //add transaction hash to signData
		for (int i = 0; i < outputIndex.length; i++)
			sigData.add(outputIndex[i]); //add index to the data 
		...
		for (Byte sb : sigData)
			sigD[i++] = sb;
		return sigD; //return a copy of the signData
	}

	/**
	 * It adds the signature to a particular input transaction
	 * 
	 * @param signature
	 * @param index
	 */
	...
	/**
	 * This method concatenates all the input and output information of this
	 * transaction in a byte array
	 * 
	 * @return
	 */
	...
	/**
	 * This method gets the concatenated value of the transaction and prepares
	 * its hash value using sha256
	 */
